...
function convertClinicTextsToJson() {
    ...
    // 症例（CASE）フィールドの取り込み
    // CSVに case1-1, case1-2, case1-3 ... として追加された行を各クリニックに保存
    try {
        records.forEach(row => {
            const listName = row[0];
            const fieldName = row[1];
            if (!listName || !fieldName) return;
            if (/^case[1-3]-[1-3]$/.test(listName)) {
                for (let j = 0; j < clinicNames.length; j++) {
                    const clinicName = clinicNames[j];
                    if (clinicName && clinicName.trim() && result[clinicName]) {
                        const value = row[j + 3] || '';
                        result[clinicName][fieldName] = value;
                    }
                }
            }
        });
